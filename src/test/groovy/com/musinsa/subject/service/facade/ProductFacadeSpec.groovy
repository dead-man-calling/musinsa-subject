...
class ProductFacadeSpec extends Specification {
    ...
    def "createProduct - 브랜드가 없을 경우 예외가 발생한다"() {
        ...
        and:
        brandService.getBrand(nonExistentBrandId) >> { throw new NoSuchElementException("No value present") }
    ...
    def "createProduct - 카테고리가 없을 경우 예외가 발생한다"() {
        ...
        and:
        brandService.getBrand(brandId) >> brand
        categoryService.getCategory(nonExistentCategoryId) >> { throw new NoSuchElementException("No value present") }
